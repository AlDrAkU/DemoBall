...
// Variables
struct Ball {
Vector2f Velocity{ 100.0f,50.0f };
...
const float g_Gravity{ -9.81 * 100 }; // m/sec2 
...
void InitGameResources( )
{
	for (int idx{ 0 }; idx < g_NumBalls; ++idx)
	{
		...
		g_Balls[idx].Velocity.x = rand() % (151 - 50) + 50.f;
		g_Balls[idx].Velocity.y = rand() % (151 - 50) + 50.f;
		...
		//g_Balls[idx].Color.r = 1.0f;
		g_Balls[idx].Color.r = 0.0f;
		g_Balls[idx].Color.g = 0.0f;
		g_Balls[idx].Color.b = 1.0f;
...
void Update( float elapsedSec )
{
	...
	// calculate new pos using the velocity 
...
{
	ClearBackground( );
	/*FillCircle(Point2f{ g_WindowWidth / 2,g_WindowHeight / 2 }, 25, Color4f{1.0f,1.0f,1.0f,1.0f});*/
